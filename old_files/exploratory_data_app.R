...
# Prevent raw error messages from appearing in UI
...
preprocess_numeric <- function(x, impute = "median", cap_outliers = TRUE) {
  safeRun({
    if (impute == "median") {
    ...
    if (cap_outliers) {
      ...
      x <- ifelse(x < lower_bound, lower_bound, ifelse(x > upper_bound, upper_bound, x))
    }
    return(x)
  })
}
...
ui <- fluidPage(
  ...
  navbarPage("",
             ...
             tabPanel("User Guide",
                      div(class = "container",
                          h3("Overview"),
                          p("This app lets you upload datasets (CSV, Excel, JSON, RDS or built-in), clean and preprocess data (including converting numeric-like text columns to numeric), engineer features, and perform exploratory data analysis (EDA) with dynamic plots."),
                          h4("Key Features:"),
                          tags$ul(
                            tags$li("Data Upload: Supports CSV, Excel, JSON, and RDS files"),
                            tags$li("Data Cleaning: Remove duplicates, handle missing values, outliers, normalize/standardize, encode categorical variables, and convert numeric-like columns"),
                            tags$li("Feature Engineering: Apply mathematical transformations, create custom columns, and rename columns"),
                            tags$li("EDA: Generate interactive histograms, boxplots, bar charts, scatter plots, and correlation heatmaps"),
                            tags$li("Download: Export the cleaned data")
                          ),
                          h4("Navigation:"),
                          p("Use the tabs above to move between Data Upload, Data Cleaning, Feature Engineering, EDA, and Download & Reset.")
                      )
             ),
             ...
             tabPanel("Data Cleaning & Preprocessing",
                      sidebarLayout(
                        ...
                        mainPanel(
                          fluidRow(
                            column(4,
                                   div(class = "container", h4("Cleaning Summary"), uiOutput("cleaningSummary"))
                            ),
                            column(8,
                                   div(class = "container",
                                       tabsetPanel(
             ...
             tabPanel("Feature Engineering",
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          numericInput("numCustom", "Number of custom columns to create:", value = 0, min = 0, max = 50),
                          uiOutput("customColUI"),
                          hr(),
                          numericInput("numRename", "Number of columns to rename:", value = 0, min = 0, step = 1),
                          uiOutput("renameUI"),
                          hr(),
                          h4("Additional Feature Engineering"),
                          checkboxInput("extractTimeFeatures", "Extract Time Features", value = FALSE),
                          checkboxInput("extractTextFeatures", "Extract Text Features (Word/Character Count)", value = FALSE),
                          ...
                          hr(),
                          actionButton("applyFEAll", "Apply Feature Engineering Changes", class = "btn btn-primary"),
                          actionButton("resetFE", "Reset FE Inputs", class = "btn btn-default", style = "margin-left:20px;")
                        ),
                        mainPanel(
                          div(class = "container",
...
server <- function(input, output, session) {
      ...
      if (input$dataSource == "upload") {
        ...
        for (i in seq_len(nrow(files))) {
          ...
          if (ext == "csv") {
            ...
            data <- tryCatch(
              fromJSON(files$datapath[i], flatten = TRUE),
              error = function(e) {
                con <- file(files$datapath[i], open = "r")
        ...
        rv$datasets <- lapply(input$builtinDataset, function(ds) {
          data <- get(ds)
          if (!is.data.frame(data)) data <- as.data.frame(data)
          data
        })
        names(rv$datasets) <- input$builtinDataset
        rv$cleaned <- rv$datasets
      }
  ...
  output$dataPreview <- renderDT({
    safeRun({
      if (is.null(input$uploadPreview) || input$uploadPreview == "") return(datatable(data.frame()))
      d <- rv$datasets[[input$uploadPreview]]
      col_types <- jsonlite::toJSON(unname(sapply(d, function(x) paste(class(x), collapse = ", "))))
      datatable(d,
                options = list(scrollX = TRUE, scrollY = "300px", paging = TRUE, pageLength = 10),
                callback = JS(sprintf("
                  var colTypes = %s;
                  table.columns().header().to$().each(function(i, th) {
                    $(th).attr('title', 'Type: ' + colTypes[i]);
                  });
                ", col_types))
      )
    })
  ...
  output$beforeCleanPreview <- renderDT({
    safeRun({
      ...
      d <- rv$datasets[[input$activeDataset]]
      col_types <- jsonlite::toJSON(unname(sapply(d, function(x) paste(class(x), collapse = ", "))))
      datatable(d,
                options = list(scrollX = TRUE, scrollY = "300px", paging = TRUE, pageLength = 10),
                callback = JS(sprintf("
  ...
  output$cleanPreview <- renderDT({
    safeRun({
      req(input$activeDataset)
      d <- rv$cleaned[[input$activeDataset]]
      col_types <- jsonlite::toJSON(unname(sapply(d, function(x) paste(class(x), collapse = ", "))))
      datatable(d,
                options = list(scrollX = TRUE, scrollY = "300px", paging = TRUE, pageLength = 10),
                callback = JS(sprintf("
  ...
  observeEvent(input$clean, {
    safeRun({
      ...
      # Convert numeric-like character columns if selected
      if (input$convertNumeric) {
        data <- data %>% mutate(across(where(is.character), ~ {
          conv <- suppressWarnings(as.numeric(.))
          if (mean(!is.na(conv)) >= 0.5) conv else .
        }))
      }
      ...
      if (!is.null(input$colsToRemove) && length(input$colsToRemove) > 0) {
        data <- data[, !names(data) %in% input$colsToRemove, drop = FALSE]
      }
  ...
  observeEvent(input$activeDatasetFE, {
    safeRun({
      ...
      if (length(num_cols) == 0) {
        showNotification("No numeric columns available in cleaned data.", type = "warning")
    ...
    safeRun({
      ...
      if (!is.null(input$mathTransform) && length(input$mathTransform) > 0) {
        if ("log" %in% input$mathTransform) {
          req(input$cols_log)
          for (col in input$cols_log) {
            new_val <- log(data[[col]] + 1)
            if (isTRUE(input$round_log)) new_val <- round(new_val, digits = input$roundDigits_log)
            data[[paste0("log_", col)]] <- new_val
          }
        }
        if ("sqrt" %in% input$mathTransform) {
          req(input$cols_sqrt)
          for (col in input$cols_sqrt) {
            new_val <- sqrt(data[[col]])
            if (isTRUE(input$round_sqrt)) new_val <- round(new_val, digits = input$roundDigits_sqrt)
            data[[paste0("sqrt_", col)]] <- new_val
          }
        }
        if ("square" %in% input$mathTransform) {
          req(input$cols_square)
          for (col in input$cols_square) {
            new_val <- data[[col]]^2
            if (isTRUE(input$round_square)) new_val <- round(new_val, digits = input$roundDigits_square)
            data[[paste0("square_", col)]] <- new_val
          }
        }
        if ("boxcox" %in% input$mathTransform) {
          req(input$cols_boxcox)
          for (col in input$cols_boxcox) {
            ...
            new_val <- if (abs(lambda) < .Machine$double.eps) log(x_shifted) else (x_shifted^lambda - 1) / lambda
            if (isTRUE(input$round_boxcox)) new_val <- round(new_val, digits = input$roundDigits_boxcox)
            data[[paste0("boxcox_", col)]] <- new_val
          }
        }
        if ("power" %in% input$mathTransform) {
          req(input$cols_power)
          for (col in input$cols_power) {
            exponent <- input$exponent_power
            new_val <- data[[col]]^exponent
            if (isTRUE(input$round_power)) new_val <- round(new_val, digits = input$roundDigits_power)
      ...
      if (!is.null(input$featureSelection)) {
        if ("var_thresh" %in% input$featureSelection) {
          ...
          if (length(low_var_cols) > 0) {
            data <- data[, !names(data) %in% low_var_cols, drop = FALSE]
          }
        }
  ...
  output$varSelectUI <- renderUI({
    safeRun({
      ...
      if (input$plotType == "hist") {
        selectInput("histVar", "Select Variable:", choices = names(data))
      } else if (input$plotType == "boxplot") {
        selectInput("boxVars", "Select Variables:", choices = names(data), multiple = TRUE)
      } else if (input$plotType == "bar") {
        selectInput("barVars", "Select Variables:", choices = names(data), multiple = TRUE)
      } else if (input$plotType == "scatter") {
        tagList(
          selectInput("scatterX", "Select X Variable:", choices = names(data)),
          selectInput("scatterY", "Select Y Variable:", choices = names(data))
        )
      } else {
        return(NULL)
      }
  ...
  observeEvent(input$plotData, {
    safeRun({
      ...
      output$statsSummary <- renderPrint({ summary(data) })
    })
  })
