...
preprocess_numeric <- function(x, impute = "median", cap_outliers = TRUE) {
  safeRun({
    ...
    if (cap_outliers) {
      ...
      x <- ifelse(x < lower_bound, lower_bound,
  ...
  navbarPage("",
             ...
             tabPanel("User Guide",
                      fluidRow(
                        column(12,
                               h3("Overview"),
                               p("This interactive web application allows you to upload and compare multiple datasets (CSV, Excel, JSON, or RDS) or use built‐in ones. You can clean, preprocess, engineer features, and explore the data through dynamic visualizations."),
                               h4("Key Features:"),
                               tags$ul(
                                 tags$li("Multiple dataset support (upload or use built‐in datasets)"),
                                 tags$li("Advanced data cleaning: remove duplicates, handle missing values, handle outliers, normalize/standardize, and encode categorical variables"),
                                 tags$li("Feature Engineering via mathematical transformations (Log, Square Root, Square, Box‑Cox, Power), custom column creation, and column renaming. Engineered features can be included in EDA."),
                                 tags$li("Interactive Exploratory Data Analysis (EDA) with dynamic variable selection based on plot type"),
                                 tags$li("Download processed data and reset the app")
                               ),
                               h4("Navigation:"),
                               p("Use the tabs below to navigate between Data Upload, Cleaning, Feature Engineering, EDA, and Download & Reset.")
                        )
                      )
             ),
             ...
             tabPanel("Data Upload",
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          br(), br(),
                          p("Hover over any column header to see its datatype.")
                        ),
                        mainPanel(
                          div(class = "container",
...
server <- function(input, output, session) {
  ...
  output$dataPreview <- renderDT({ 
    safeRun({
      if (is.null(input$uploadPreview) || input$uploadPreview == "") {
  ...
  output$beforeCleanPreview <- renderDT({ 
    safeRun({
      if (is.null(input$activeDataset) || input$activeDataset == "") {
        return(datatable(data.frame()))
      }
      datatable(rv$datasets[[input$activeDataset]], 
                options = list(scrollX = TRUE, scrollY = "300px", paging = TRUE, pageLength = 10))
    })
  })
  ...
  output$cleanPreview <- renderDT({ 
    safeRun({
      ...
      datatable(rv$cleaned[[input$activeDataset]], 
                options = list(scrollX = TRUE, scrollY = "300px", paging = TRUE, pageLength = 10))
    })
  })
